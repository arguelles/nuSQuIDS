...
int main(){
  ...
  const double lenght = 50.*units.km;
  ...
  for ( int ei = 0 ; ei < nus.GetNumE(); ei++){
	...
	inistate_nus[ei][rho][flv] = ND_flux_numode[ei][rho*3+flv+1];
	inistate_anus[ei][rho][flv] = ND_flux_anumode[ei][rho*3+flv+1];
      }
    }
  }
  ...
  for(unsigned int i = 0 ; i < nus.GetNumE(); i++){
      ...
      for(unsigned int k = 0; k < nus.GetNumNeu(); k ++){
        file << nus.EvalFlavorAtNode(k,i,0) << " ";
      }
      for(unsigned int k = 0; k < nus.GetNumNeu(); k ++){
        file << nus.EvalFlavorAtNode(k,i,1) << " ";
      }
      file << std::endl;
  }
  file.close();
