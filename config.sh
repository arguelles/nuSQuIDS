...
libdir=${HDF5_LIBDIR}
includedir=${HDF5_INCDIR}" > hdf5.pc
echo '
Name: HDF5
Description: "A data model, library, and file format for storing and managing data."
URL: https://www.hdfgroup.org/HDF5/' >> hdf5.pc
echo "Version: $HDF5_VERSION" >> hdf5.pc
...
install: $(DYN_PRODUCT) $(STAT_PRODUCT)
	...
	@cp nusquids.pc $(PREFIX)/lib/pkgconfig' >> ./Makefile
if `pkg-config hdf5`; then
:
else
	echo '	@cp hdf5.pc $(PREFIX)/lib/pkgconfig' >> ./Makefile
fi
