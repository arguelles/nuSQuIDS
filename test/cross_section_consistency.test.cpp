    ...
    for(unsigned int flv = 0; flv < numneu; flv++){
      for(unsigned int ei = 1; ei < nus.GetNumE(); ei++){
        ...
        for(unsigned int ef = 0; ef < ei; ef++){
          total_dNdECC += is->dNdE_CC[rho][flv][ei][ef]*(energies[ef+1]-energies[ef]);
        }
        if(std::abs(total_dNdECC + endcap_dNdECC - 1.0) > 1.0e-2)
        ...
        if(std::abs(total_dNdENC + endcap_dNdENC - 1.0) > 1.0e-2)
