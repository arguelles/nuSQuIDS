...
int main()
{
  ...
  nuSQUIDSLV nus(logspace(Emin,Emax,200),numneu,antineutrino,false);
  nuSQUIDSLV nus_zero(logspace(Emin,Emax,200),numneu,antineutrino,false);

  gsl_complex zero {0.0*units.eV,0.0*units.eV};
  LVParameters null {zero, zero};
  nus.Set_LV_OpMatrix(null);

  gsl_complex cmutau {1.e-20*units.eV,0.0*units.eV}
  LVParameters non_null_lv{ zero, cmutau };
  nus.Set_LV_OpMatrix(non_null_lv);
  ...
  nus_zero.Set_Body(earth);
  nus_zero.Set_Track(track);
