...
important they are no longer applicable.
...
Beyond these general purpose codes, packages have also been developed to propagate specifically tau neutrinos, \textit{e.g.} \texttt{NuTAuSim}~\cite{Alvarez-Muniz:2018owm} and \texttt{nuPyProp}~\cite{NuSpaceSim:2021hgs}.
However, none of the aboved mentioned packaged are able to take into account simultaneously 
coherent and non-coherent neutrino interactions.
...
\subsection{Standard oscillations in matter and vaccum}

{\ttf nuSQuIDS} is able to compute the neutrino oscillation probability in the presence of matter.
The neutrino oscillation can be computed for up to six neutrino flavors out of the box, though it can be extended to an arbitrary number of
neutrino flavors by producing additional tables using the provided {\ttf Mathematica} files.
Additionally, the calculation allows to set both the mixing angles and the CP-violating phases.
...
Default media, such as the Earth, the Sun, a constant density slab, and others are avaialble for prompt use.
In general, no constraint exist in the trajectory of the neutirno oscillation, though in the provided implementations all neutrinos are assumed
...
implemented in {\ttf nuSQuIDS} is unique and provides advantages over other implementation.
By working in the neutrino interacttion basis the fast oscillations due the the vacuum part of the neutrino Hamiltonian are solved analytically.
...
A unique feature of {\ttf nuSQuIDS} is that it solves the neutrino oscillation problem over a grid of energies;
though solving the problem using an individual fix neutrino energy is also possible, as in other software.
In this operating mode, {\ttf nuSQuIDS} can interpolate the interaction-picture solutions of the neutrino evolution problem for energy values
that are in between the nodes where the calculation is performed.
This allows accurate and fast evaluation of the neutrino oscillation probability using a coarse grid and efficient.
...
when reweighting large Monte Carlo data sets.
...
however, this can be extended to include additional terms that may appear on the Hamiltonian.
This can be achieved as illustrated in examples~\ref{}, where the user creates an inherited class of the main {\ttf nuSQuIDS} class and implements
the desired new physics operators.
In order to write the operators the user needs to write them using the SUN vectors, which represent arbitrary hermitian matrices.
Additionally, {\ttf SQuIDS} implements operations between matrices and scalars that typically arise in writing these terms,
\textit{e.g.} matrix-scalar multipication, conmutator between matrices, \textit{etc}.
Once the new hamiltonian is defined by the user no further change is needed. The user will be able to use all the funcionality
of {\ttf nuSQuIDS} with the new physics.
Finally, {\ttf nuSQuIDS} allows for the serialization of user-defined values which allow for new physics operator definiting constants to be serialized as
...
This funcionality is comparable to other neutrino propagators used in neutrino telescopes~\cite{Safa:2021ghs,Garcia:2020jwr,Vincent:2017svp,Yoshida:2003js,shigeruyoshida_2020_4018117,Gazizov:2004va,DeYoung:865626,Alvarez-Muniz:2018owm,NuSpaceSim:2021hgs}.
...
{\ttf nuSQuIDS} takes into account charged-current and neutral current interactions with the nucleus through deep-inelastic scattering, which is the dominant interaction at the relevant energies~\citep{Formaggio:2012cpf,Gandhi:1998ri,Zhou:2019frk,Zhou:2019vxt,CooperSarkar:2011pa}.
Additionally, we take into account neutrino-electron scattering, which is only relevant for resonant electron-antineutrino scattering at neutrino energies close to 6.4~PeV~\cite{Glashow:1960zz}.
A subdominant contribution from neutrino-photon scattering is not implemented~\cite{Seckel:1997kk,Alikhanov:2014uja,Alikhanov:2015kla,Zhou:2019frk,Zhou:2019vxt}, but can be easily extended in our framework; see~\cite{} for a publicly available implementation of this cross section~\cite{Garcia:2020jwr}.
Finally, the production of neutrinos from lepton decay is neglected for all flavors, excepto for tau neutrinos.
In this case, we account for the contribution of neutrinos from tau decay in the approximation that the tau decays promptly so that it does not undergo significant energy losses.
...
Though, since the neutrino oscillation and interaction scales are much larger that the production region or detector size, in most scenarios, we can consider the neutrino emission to be point like to good approximation.
However, there are scenarios where this approximation breaksdown.
For example, for neutrinos produced from dark matter decay through an intermediate mediator, an scenario known as secluded dark matter~\cite{Pospelov:2007mp}, the neutrinos are produced according to an exponential distribution that can be as large as the radius of the Earth or Sun depending on the model parameters~\cite{Niblaeus:2019gjk,Liu:2020ckq}.
...
Another scenario that is relevant is that of production of neutrinos in pion or kaon decay in flight, \textit{e.g.} in accelerator neutrino experiments or in cosmic-ray airshowers.
...
Neutrino oscillations can be extremely fastly varying with the neutrino energy, specially at small energies where the neutrino Hamiltonian in vaccum is large. 
...
The evaluation of oscillations on a grid of notes that has been already computed has been significantly optimized making it such that it possible to evaluate the oscillation probability for large Monte Carlo data sets much faster than computing the individual oscillation probabilities at each of the Monte Carlo event energies.
...
Then later when the user wants to evaluate the result on specific energies, they can recovere the object and perform the necesary evaluations.

\subsection{{\ttf C++} and {\ttf Python} interfaces}

{\ttf nuSQuIDS} has been written in {\ttf C++} sinze this compile-time language provides the best execution time. 
This is because internally {\ttf nuSQuIDS} operations are written in a way that they can be promptly optimize by the compiler.
In particular, the algebra of SU(N) operators significantly benefits from this optimization.
All {\ttf nuSQuIDS} functions can be access in {\ttf C++}; however, recently, there has been great interest in designing particle physics software in interpreted languages such as {\ttf Python}.
To facilitate the usage of {\ttf nuSQuIDS} with {\ttf Python}-based physics software, we have implemented an interface to {\ttf Python} by means of the Boost library.
Most of the standards {\ttf nuSQuIDS} funcionalities are available through the {\ttf Python} interface, with the notable exception of the extended body object.
This is becasue the extended body requires the user to provide a function that describes the yield of neutrinos at a given position of the track, which if written in {\ttf Python} would produce a significant amount of overhead.
For similar reasons, extensions to {\ttf nuSQuIDS} by the user are strongly encouraged to be performed in {\ttf C++} since,\textit{e.g.} adding new terms in the Hamiltonian will require calling the user-defined functions very often.
...
Via its add-on system the package can treat new physics effects, including sterile neutrinos via the {\ttf snu} add-on. 
...
For this reason it uses a Monte-Carlo sampling approach, and not only simulates neutrinos passing through the bulk of the Earth, with a continuously parameterized density profile, but also includes forcing final interactions in a volume defined as the vicinity of a detector. (This volume typically being larger than the instrumented volume of the detector in order to fully sample muons produced by charge-current $\nu_\mu$ interactions entering the detector from outside.)
While ANIS treats all deep-inelastic scattering and Glashow resonance (resonant $W^\pm$ production) processes as well as tau propagation and decay, it does not calculate oscillations. 
DIS The code requires DIS cross sections in the somewhat unusual form of Bjorken x, y pairs, pre-sampled at natural frequencies from the desired cross section. 
...
It has the advantage that it has less dependence to other software and allows for arbitrary media and trajectories.
It also uses a Monte-Carlo sampling method, and does not treat oscillations.
It can use several build-in cross section tables that can be extended by the user.
...
This body includes botht he Earth and its atmosphere. 
...
associated with an isoscalar nucleon targer: one with only charge current  interactions
...
Some neutrino evolution problem cannot be formulated as an starting flux
of neutrinos, as shown in the examples above.
...
nuSQUIDS provides a way to handle these scenarios, which we will illustrate in this example.

A source term is added to the nuSQuIDS kinetic equations Eq.~\eqref{eq:kinetic_equations},
which allows to add (or substract) neutrinos from the ensemble.
This term depends on the location of the ensemble in the traversed body.
For this reason it is in the body class where the user needs to implement this term; namely it is
a property of the objected been traversed rather than the ensemble.
...
The flavor is specified in the standard nuSQUIDS flavor notation: 0 for electrons, 1 for muons, and 2 for taus, etc.
The injection of neutrinos in our example will follow an exponential emission energy profile, namely
\begin{equation}
  S(t) = \Pi_\alpha \exp(-x/L),
\end{equation}
where $\Pi_\alpha$ represents the projector of the neutrino state of flavor $\alpha$, $x$ is the distance along the trayectory,  and $L$ is our exponential decay length.
For a given body to emit neutrinos the following member function with the following signature must be overriden:
\begin{lstlisting}[frame=leftline, numbers =
  left,breaklines=true,label = ex:sin1,firstnumber=last]
    void injected_neutrino_flux(marray<double,3>& flux, const GenericTrack& track, const nuSQUIDS& nusquids) override
\end{lstlisting}
The user is given a container, \texttt{flux}, which has three indexes that correspond to the energy index, neutrino or antineutrino index, and flavor index.
Additionally, the user is given constant references of a track and the nuSQUIDS object that query the body.
The user can use the track information to find the current position in the ensemble evolution and can use the nuSQUIDS object
to obtain additional information that they may need to fill in the container.
For example, the user may need access to the energy nodes of the calculation, which can be obtained by calling the corresponding member function of the nuSQUIDS class.
It is important to note that the type of track that is passed by nuSQUIDS to the body object member function is a \texttt{GenericTrack}, which is necesarry internally
so that this operation can work with all the body types. If needed, in the implementation of the member function the user can cast this to the track type they prefer.
...
internally by nuSQUIDS. This keyword will trigger a compile-time error, if the user accidentally declares a new member function and thus guarantees that the correct member function
is been overriden.

Once your body has been constructed and pass to nuSQUIDS by means of the usual \texttt{Set\_Body} and \texttt{Set\_Track} member functions, we need to inform nuSQUIDS that we will be considering neutrino injection into the ensemble in the calculation.
...
\end{lstlisting}
where here \texttt{nus} corresponds to the instance of the nuSQUIDS object that we have created.
...
These operations are in turn used by several of the key portions of nuSQuIDS, and for oscillation-only calculations evaluation of trigonometric functions can be the single largest part of time consumed by the library. 
As a result, run time is significantly dependent on the performance of the underlying math library, which is usually supplied by the operating system, and sometimes the compiler. 
...
The approximately factor of two difference in fused sine/cosine performance between glibc 2.17 and 2.28 translates into a 10-25\% difference in the overall runtime of nuSQuIDS oscillation calculations. 
The user is therefore cautioned that the choice of compiler and runtime libraries can be important to maximizing the performance of nuSQuIDs. 
...
It is difficult to give specific guidance for all of the many types of problems to which nuSQuIDS is applied, so users are advised to be prepared to perform their own tests of how many nodes are needed to get good results for their use cases. 
...
As an example, a modestly sized 3 flavor neutrino oscillation calculation might use 100 energy nodes and not consider non-coherent interactions, so $M=100$, $N=3$, $P=2$ (if both neutrinos and anti-neutrinos are considered), and $T$ is irrelevant. 
...
On simple trick is to turn off features which are not needed for a given calculation. 
...
in Table~\ref{tab:hdf_cross_layout}. All cross sections must be tabulated with the same set of incident neutrino energies, and each singly-differential cross section must be tabulated for the same set of out-going lepton energies, expressed in the $z$ variable described in Sec.~\ref{ssssec:differential_tabulation}. 
...
\subsubsubsection{Singly-Differential Cross Section Tabulation Details}
\label{ssssec:differential_tabulation}
...
Since the shape of the peak itself cannot be represented faithfully, we choose to attempt to preserve its integral instead, and choose to replace the final entry in the two with one selected so that the average of the last two values is equal to the true average computed from a detailed integral of the cross section over the bin. 
