...
In this example we show a simple extension of the default
...
\subsection{ BSM extension: Non-Standard  Interaction \textnormal{({\ttf
      examples/NSI})}}
Neutrino oscillations are a good prove for new physics. In this
example we illustrate how to construct a derived class of
nusquids adding new physics modification. The procedure is similar to
other new physics setups.

The implementation of {\ttf nuSQUIDSNSI} is done in {\ttf NSI.h}.
...
called {\ttf NSI}\. Since we are working in the interaction picture we need
to define a matter potential rotated with $H_0$ for every energy node.
This will be stored in {\ttf NSI\_evol}.
...
diagonal term. Notice that we set the NSI diagonal contribution to
zero.
...
Before computing any derivative in the r.h.s of the differential equation
we should have the NSI operator in the interaction picture. We compute
this for every node in prederive.
...
This  auxiliary functions allow the nuSQuIDSNSI to
save the new information that defines the new derived object in
the hdf5 files.
...
Here we overload the {\ttf HI} function where we return the full
matter potential.
...
nuSQUIDS constructor and after sets the standard oscillation
...
This last function sets the value of {\ttf epsilon\_mutau} changing the
...
propagate with {\ttf  epsilon\_mutau=0} and {\ttf
  epsilon\_mutau=1e-2}. 
...
After setting the parameters we propagate both cases, and put both
...
a compact way to treat a set of nuSQUIDS objects distributed in zenith that
allows us to solve the problem of propagating the full, energy and
...
After setting the zenith array the rest of the arguments are the same
as the basic nuSQUIDS object.
...
In this case we need to set the initial state also depending on
zenith. We fill the multi-dimensional array with the initial state of the
...
Finally we save the flux in a text file. The
