...
In the case of the square mass difference,  we the first argument of the function is
the mass eigenvalue index and the second argument the value.
  ...
  \end{lstlisting}
      Function that given an incident neutrino energy ({\ttf E1}) in eV an outgoing lepton energy ({\ttf E2}), as well as neutrino flavor,
       ...
       respect to the outgoing lepton energy in ${\rm cm}^2/{\rm
         GeV}$. This function interpolates the cross section from a reference table by means of aa bi-linear interpolation.
\end{itemize}
...
\begin{itemize}
  ...
  specify if and where in the HDF5 file structure will the cross sections be saved
  ...
  functions has to be written consistently with {\ttf
...
the same names as in the {\ttf C++} interface. Additionally, two classes from SQuIDS have been expose to {\ttf Python}:
\begin{itemize}
...
import nuSQUIDSTools
\end{lstlisting}
Through the example we will use the shorthand {\ttf nsq} for the {\ttf nuSQUIDS} python bindings.
...
We can propage a neutrino ensemble through the Earth in an atmospheric neutrino telescope setting.
